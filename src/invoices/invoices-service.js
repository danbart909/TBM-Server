const InvoiceService = {
  ...
  getCurrentCartId(knex, user_id) {
    console.log("GET CURRENT CART ID *******", user_id)
    return knex
      .select("*")
      .from("invoices")
  ...
  insertInvoice(knex, invoice_id, product_id, user_id, quantity) {
    console.log(invoice_id, product_id, user_id, quantity)
    return knex
      .insert(invoice_id, product_id, user_id, quantity)
      .into("invoice_products")
      .returning("*")
      .then((rows) => {
        return rows[0];
      });
